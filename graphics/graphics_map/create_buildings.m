...
%
    ...
    map.buildings_heights = 2*rand(map.nb_blocks*map.nb_blocks,1);%map.max_height*rand(map.nb_blocks*map.nb_blocks,1);
    
    for i=1:map.nb_blocks
        buildings_north(i) = [1+(i-1)*100];%[0.5*map.width/map.nb_blocks*(2*(i-1)+1)];
    end
