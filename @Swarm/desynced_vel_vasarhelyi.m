function [vel_command, collisions] = desynced_vel_vasarhelyi(self, p_swarm, r_agent, dt)
    ...
    %% ========================================
    %% ===== Attack-Related Modifications =====
    attack_time = 10; % in seconds -> 100*attack_time => in frames
    desync_frame = 1000; % in frames
    attack_vel = true;
    attack_pos = true;
    
    %% ===== Velocity Delay Term =====
    delayed_vel = zeros(3, nb_agents);
    if length(self.collisions_history) > attack_time*100 && attack_vel
        % if the simulation frame is after attack trigger time, then delay the passed velocity data
        for agent = 1:nb_agents
            delayed_vel(:,agent) = self.drones(agent,1).vel_xyz_history(end - desync_frame, :);
        end
    else
        % if the current timestamp is before trigger time, then pass the current velocity data
        delayed_vel = vel;
    end

    %% ===== Position Delay Term =====
    delayed_pos = zeros(3, nb_agents);
    if length(self.collisions_history) > attack_time*100 && attack_pos
        for agent = 1:nb_agents
            delayed_pos(:,agent) = self.drones(agent,1).pos_ned_history(end - desync_frame, :);
        end
    else
        delayed_pos = pos;
    end
    ...
    for agent = 1:nb_agents
        ...
        if nb_neig ~= 0
            v_rel = delayed_vel - vel(:, agent);
            v_rel(:, agent) = zeros(3, 1);
            % v_rel = vel - vel(:, agent); % original code
